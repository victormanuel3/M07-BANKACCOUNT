...
class WithdrawTransaction extends BaseTransaction implements BankTransactionInterface{
    ...
    function applyTransaction(BackAccountInterface $account) : float{
        ...
        if ($newBalance < 0) { //En caso de que la resta sea menor a 0 ya no verificamos el NoVerdraft sino el silver directamente
            if ($overdraft) { //Overdraft me devuelve true o false en caso de que el saldo final sea menor a -100, sino NO es menor a -100 retornamos el saldo.
                return $newBalance;
            }else {
                throw new InvalidOverdraftFundsException("Insufficient balance to complete the withdrawal.");//Excepción: Si es menor a -100 no podemos restar.
            }
        }else {
